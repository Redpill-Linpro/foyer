...
Rails::Initializer.run do |config|
  ...
  # This application uses Bundler. Add gems to Gemfile, and install them using
  # "bundle install".
...
# Make general config data available globally:
GENERAL = Hash.new { |hash, key| hash[key] = {} }
GENERAL.merge!( YAML::load( File.read( File.join(RAILS_ROOT, "config", "general.yml") )) )
GENERAL.freeze

# Add extra functions to migrations/schema.rb.
ActiveRecord::ConnectionAdapters::AbstractAdapter.send(:include, DBHelper)

# Make methods in MigrationHelper available in migrations:
ActiveRecord::Migration.send :include, MigrationHelper

# Monkeypatch required after migration 015 in order for db:schema:dump to work
# properly:
